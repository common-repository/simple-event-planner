...
class Simple_Event_Planner_Meta_Box_Event_Options {
                                            ...
                                            <div id="itemRows">
                                                ...
                                                if(is_array($seg_array))
                                                {
                                                    foreach ($seg_array as $key => $value) { ?>
                                                        <p id="rowNum<?php echo esc_attr( intval( $row_count ) ); ?>">
    ...
    public static function sep_save_event_listing_meta($post_id) {
        ...
        if (isset($_POST['custom'])) {
            foreach ($_POST['custom'] as $key => $value) {
                if (sanitize_text_field($key) !== ""
                    ...
                    $sxe->addChild(sanitize_key($key), empty($value) ? '' : $value );
                }
            }
        }
        ...
        $add_seg = ( empty( $_POST['custom']['add_seg'] ) ) ? $_POST['custom']['add_seg'] = " " : array_map( 'sanitize_text_field', $_POST['custom']['add_seg']);  
        $serialized_add_seg = serialize( $add_seg ); 
        
        update_post_meta( $post_id, 'add_segment', ( $serialized_add_seg ) ); // phpcs.ignore - Data is sanitized before serialization
    ...
    private static function save_google_map($sxe, $id = '') {
        ...
        $sxe->addChild('event_address', $event_address); // phpcs.ignore - Sanitized on line 416
        $sxe->addChild('loc_lat', $loc_lat); // phpcs.ignore - Sanitized on line 417
        $sxe->addChild('loc_long', $loc_long); // phpcs.ignore - Sanitized on line 418
        $sxe->addChild('loc_zoom', $loc_zoom); // phpcs.ignore - Sanitized on line 419
        $sxe->addChild('loc_map', $loc_map); // phpcs.ignore - Sanitized on line 420
